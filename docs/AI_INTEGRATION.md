...
- **Dados sensíveis proibidos:** Nenhuma informação contabilística ou dados pessoais podem ser enviados para a API externa. O módulo deve funcionar de forma isolada e segura.
...
- O utilizador selecciona um contexto contabilístico específico (ex.: empresa, módulo, contas, documentos ou outro âmbito disponível no sistema).
...
Permitir que o utilizador faça perguntas sobre os seus dados contabilísticos, como análises, resumos, comparações e explicações, garantindo que as respostas sejam baseadas exclusivamente nos dados existentes no sistema.
